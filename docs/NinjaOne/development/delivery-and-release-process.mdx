...
2. **Run cross-platform smoke tests on macOS and Linux with PowerShell 7**
3. **Run the full Windows test job**
4. **Build the module and publish artifacts**
5. **Sync development docs after successful push runs**

The cross-platform smoke job runs a lightweight `core,private` Pester subset on `ubuntu-latest` and `macos-latest` using `pwsh`.

The Windows test job also bootstraps dependencies, builds the C# helper projects, uploads Pester results, and publishes coverage artifacts to Codecov.
